...
print('model07_self_Attention_ln-lstm')
...
class SiameseCSNN(object):
    ...
    def atten_all_wrapper(self, input_q, input_c,scope):
        ...
        with tf.device('/gpu:0'):
            ...
            self.M = M = tf.squeeze(tf.einsum('abcd,df->abcf',tf.tanh(tf.add(re_fq,re_fc)),V),[3])
            #  查询在代码上注意力 (?,15,200)
    ...
    def build(self, embeddings):
        ...
        with tf.variable_scope('onlstm') as scope0:
            ...
            # LSTM (?, 200, 512)
            self.sc = 0
            lstm_c_pos = self.onlstmm(c_pos_embed, self.dropout_keep_prob, "code", 2*self.hidden_size)
            # lstm_c_pos = self.call_onlstm(c_pos_embed)

            # LSTM (?, 15, 512)
            self.sc = 1
            lstm_q = self.onlstmm(q_embed, self.dropout_keep_prob, "query", 2*self.hidden_size)
            ...
            # LSTM (?, 200, 512)
            self.sc = 0
            lstm_c_neg = self.onlstmm(c_neg_embed, self.dropout_keep_prob, "code", 2*self.hidden_size)
        ...
        with tf.variable_scope('attention') as scope1:
            ...
            # 维度 (?,512,200)  (?,512,15)
            q_pos_atted, c_pos_atted = self.atten_all_wrapper(lstm_q, lstm_c_pos,'att')


            scope1.reuse_variables()  # 复制参数
            # 维度 (?,512,200)  (?,512,15)
            q_neg_atted, c_neg_atted = self.atten_all_wrapper(lstm_q, lstm_c_neg,'att')
